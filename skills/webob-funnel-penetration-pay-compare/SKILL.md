...
- Sensors credentials: read `SENSORS_OPENAPI_KEY`, `SENSORS_BASE_URL`, and `SENSORS_PROJECT` from environment variables, the current working directory `.env`, or the installed skill's `.env`; never print secrets. This skill uses Sensors OpenAPI API Key auth only. API Secret / legacy `token=` authentication is not a supported path.
...
```bash
python3 ~/.codex/skills/webob-funnel-penetration-pay-compare/scripts/compare_penetration_pay.py \
  --project bend \
  --funnels 73,288 \
  --start 2026-05-01 \
  --end 2026-05-06
...
```bash
python3 ~/.codex/skills/webob-funnel-penetration-pay-compare/scripts/diagnose_member_chain.py \
...
```bash
python3 ~/.codex/skills/webob-funnel-penetration-pay-compare/scripts/diagnose_submit_pay_breakdown.py \
  --baseline-start 2026-05-03 \
  --baseline-end 2026-05-09 \
  --compare-start 2026-05-10 \
  --compare-end 2026-05-10 \
